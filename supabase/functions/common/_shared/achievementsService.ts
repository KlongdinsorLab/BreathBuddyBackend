import {
  ...
  notExists,
  or,
} from "npm:drizzle-orm@^0.31.2/expressions";
...
export async function getLockedAchievementsSelectCharacter /* or MyBag? */(
  playerId: number,
) {
        ...
        notExists(
          db
            ...
            .where(
              and(
                eq(
                  playersAchievementsTable.achievement_id,
...
export async function getUnlockedAchievements(playerId: number) {
  // for My Bag page
  const achievements = await db
    .select({
      id: achievementsTable.id,
      ...
      games_played_consecutive_days:
        achievementsTable.games_played_consecutive_days,
...
export async function getNewAchivements(playerId: number) {
  ...
  const playersBoostersList = await db
    ...
    .innerJoin(
      boostersTable,
      eq(boostersTable.id, playersBoostersTable.booster_id),
    );
  ...
  for (let i = 0; i < lockedAchievements.length; i++) {
    const isUnlocked = await checkAchievement(
      playerId,
      lockedAchievements[i],
      gameSessionList,
...
export function checkAchievementBossEncounters(
  ...
  logger.debug(
    "Boss " +
      achievement.boss_id +
      " Required : " +
      achievement.boss_encounter +
      "\n Boss " +
      achievement.boss_id +
      " Encountered : " +
      bossGameSessionList.length,
  );
...
export function checkAchievementBoosters(
  ...
  if (achievement.booster_action === "USE") {
    playersBoostersList = playersBoostersList.filter(
      (element) => element.status === "USED",
    );
  ...
  if (achievement.booster_type !== null) {
    playersBoostersList = playersBoostersList.filter(
      (element) => element.type === achievement.booster_type,
    );
    totalNumber = playersBoostersList.length;
  }

  if (achievement.booster_unique === "UNIQUE") {
    const uniqueBoosterList = [
...
export function checkAchievementGamesPlayedInADay(
  ...
  // TODO
  if (playerId === null || achievement.games_played_in_a_day === null) {
    return false;
  }
  ...
  gameSessionList.forEach((_gameSession, index) => {
    ...
    const isSameDay = checkSameDay(
      gameSessionList[index - 1].started_at,
      gameSessionList[index].started_at,
    );
...
export function checkAchievementGamesPlayedConsecutiveDay(
  ...
  if (playerId === null || achievement.games_played_consecutive_days === null) {
    return false;
  }
...
export function checkOneDayApart(date1: Date, date2: Date): boolean {
  const date1Tomorrow = new Date(
    date1.getUTCFullYear(),
    date1.getUTCMonth(),
    date1.getUTCDate() + 1,
  );
  return (
    date1Tomorrow.getUTCFullYear() === date2.getUTCFullYear() &&
